...
fig_choropleth = px.choropleth_mapbox(
    choropleth_gdf,
    geojson=choropleth_gdf.geometry.__geo_interface__,  # automatically handles geometries
    locations=choropleth_gdf.index,  # or use 'launch_place' if unique
    color='launched',
    color_continuous_scale='Viridis',
    featureidkey='id',  # only works if `locations` match geojson "id"s (not needed if using __geo_interface__ correctly)
    mapbox_style='carto-positron',
    center={"lat": 47.8, "lon": 30.6},
    zoom=4,
    opacity=0.6,
    hover_data=None,
